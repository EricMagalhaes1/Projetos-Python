...
stock1_text = TextInput(title="Ação Principal")
stock2_text = TextInput(title="Ação Secundaria")
...
date_picker_to = DatePicker(title='End Date', value="2020-02-01", min_date="2000-01-01",
                             max_date=dt.datetime.now().strftime("%Y-%m-%d"))
indicator_choice = MultiChoice(options=["100 Dia", "30 Dia", "Linha de Regreção Linear"])

load_button = Button(label="Carregar", button_type="success")
